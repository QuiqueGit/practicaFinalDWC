...
class VehiculosMySqlDAO implements VehiculosDAO{
	...
	public function insert($vehiculo){
		$sql = 'INSERT INTO vehiculos (tipo, matricula, fecha_fabricacion, marca, modelo, id_cliente) VALUES (?, ?, ?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
	...
	public function update($vehiculo){
		$sql = 'UPDATE vehiculos SET tipo = ?, matricula = ?, fecha_fabricacion = ?, marca = ?, modelo = ?, id_cliente = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
