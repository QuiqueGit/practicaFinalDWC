...
$("div.content").on('click','i#editar', function(){
	...
	if($("#tabla").is(":visible")) {
		...
		$(this).closest('tr').each(function() {
		    ...
		    var fnac = App.fechaES_a_fechaUS($(this).find(".fnac").html());
		    var direccion = $(this).find(".direccion").html();
		    var provincia = $(this).find(".provincia").html();
		    var fechaAlta = App.fechaES_a_fechaUS($(this).find(".fechaAlta").html());
	    ...
	    var fnac = App.fechaES_a_fechaUS($(tr).find(".fnac").html());
	    var direccion = $(tr).find(".direccion").html();
	    var provincia = $(tr).find(".provincia").html();
	    var fechaAlta = App.fechaES_a_fechaUS($(tr).find(".fechaAlta").html());
...
//evento click icono COCHE en Tabla Clientes
$("div.content").on('click','i#vehiculo_icon',function(){

	var idCliente;	

	$("#tabla").is(":visible") ? idCliente = $(this).closest('tr').attr('id') : idCliente = $(this).parent().attr('id');

	events.publish('getVehiculos', idCliente);
});