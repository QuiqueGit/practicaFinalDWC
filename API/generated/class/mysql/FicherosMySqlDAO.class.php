...
class FicherosMySqlDAO implements FicherosDAO{
	...
	public function insert($fichero){
		$sql = 'INSERT INTO ficheros (tipoFichero, nombre, ruta, id_vehiculo) VALUES (?, ?, ?, ?)';
		$sqlQuery = new SqlQuery($sql);
		
		$sqlQuery->set($fichero->tipoFichero);
		$sqlQuery->set($fichero->nombre);
		$sqlQuery->set($fichero->ruta);
	...
	public function update($fichero){
		$sql = 'UPDATE ficheros SET tipoFichero = ?, nombre = ?, ruta = ?, id_vehiculo = ? WHERE id = ?';
		$sqlQuery = new SqlQuery($sql);
	...
	public function queryByTipoFichero($value){
		$sql = 'SELECT * FROM ficheros WHERE tipoFichero = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function queryByRuta($value){
		$sql = 'SELECT * FROM ficheros WHERE ruta = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->getList($sqlQuery);
	}
	...
	public function deleteByTipoFichero($value){
		$sql = 'DELETE FROM ficheros WHERE tipoFichero = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	public function deleteByRuta($value){
		$sql = 'DELETE FROM ficheros WHERE ruta = ?';
		$sqlQuery = new SqlQuery($sql);
		$sqlQuery->set($value);
		return $this->executeUpdate($sqlQuery);
	}
	...
	protected function readRow($row){
		...
		$fichero->id = $row['id'];
		$fichero->tipoFichero = $row['tipoFichero'];
		$fichero->nombre = $row['nombre'];
		$fichero->ruta = $row['ruta'];
